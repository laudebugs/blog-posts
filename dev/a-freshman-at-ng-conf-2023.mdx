...
I learned something new about the framework - how to use auxilliary routes in Angular and how to type the `ngTemplateOutletContext` directive. Speaking during breakfast and lunch with developers working at various contexts with Angular, there was this general feeling of excitement about how the framework presented an opportunity for enabling them to build better applications for their users and improving the developer experience(DX) with each release. DX in Angular is getting better as the framework is embracing tooling from the community such as:
...
Can Angular be exciting? I don't know if that's the right question to ask. More and more, applications are getting older — so can Angular provide a platform where developers can not only maintain those older applcations but improve them with each new version of Angular? As well, can Angular be the framework of choice for developers building new applications? These seem to be deeper questions that might not have a clear answer. The work to improve the framework is continuing with improvements on reactivity, performance and hydration just to name a few areas where work is already in progress.
