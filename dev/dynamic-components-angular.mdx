...
export class UserProfileHostDirective {
...
@ViewChild(UserProfileHostDirective, { static: true }) userProfileHost: UserProfileHostDirective;

/* Using ViewChildren to get the all occurrences */
@ViewChildren(UserProfileHostDirective, { static: true }) userProfileHosts: QueryList<UserProfileHostDirective>;
```
...
/* We defined the user profile host directive earlier */
import { UserProfileHostDirective } from './user-profile-host/user-profile-host.directive.ts';
...
@Component({
	selector: 'app-component',
	template: `<ng-container #UserProfileEntry></ng-container>`,
})
export class AppComponent extends OnInit, AfterViewInit {
